...
import { canRequestWhenOpen, passLimitFor, MAX_PASSES_PER_SCHOOL_DAY } from "./convex/hallPassRules.ts";

// THE PER-CHILD CAP, tested through canRequestWhenOpen rather than
// canRequest: requests are switched off school-wide since 2026-09-13
// (STUDENT_PASS_REQUESTS_OPEN), and asserting through the gated function
// would turn every rule below into a restatement of the flag. The flag has
// its own assertions in convex/hallPassRules.test.mjs.
...
check("under the limit is allowed", canRequestWhenOpen(took(1), open, NOW, 2).ok === true);
check("at the limit is refused", canRequestWhenOpen(took(2), open, NOW, 2).ok === false);
check("over the limit is refused", canRequestWhenOpen(took(5), open, NOW, 2).ok === false);
check("the refusal says the number", /2 passes/.test(canRequestWhenOpen(took(2), open, NOW, 2).reason));
check("one pass is singular", /1 pass\b/.test(canRequestWhenOpen(took(1), open, NOW, 1).reason));

console.log("\n3. Zero means no passes, and says so differently");
const zero = canRequestWhenOpen([], open, NOW, 0);
...
check("no limit, under the school cap", canRequestWhenOpen(took(3), open, NOW, null).ok === true);
check("no limit, at the school cap", canRequestWhenOpen(took(MAX_PASSES_PER_SCHOOL_DAY), open, NOW, null).ok === false);
check("undefined behaves as absent", canRequestWhenOpen(took(3), open, NOW, undefined).ok === true);
check("a personal limit ABOVE the school cap cannot raise it",
  canRequestWhenOpen(took(MAX_PASSES_PER_SCHOOL_DAY), open, NOW, 99).ok === false);

console.log("\n5. The older rules still win where they should");
check("a live pass still blocks, whatever the limit",
  canRequestWhenOpen([{ state: "out", requestedAt: NOW }], open, NOW, 99).ok === false);
