...
function Digit({ place, value, height, digitStyle }: DigitProps) {
  /**
   * WILDCAT CHANGE: the two hooks moved ABOVE the decimal-point early return.
   *
   * As published, `useSpring` and `useEffect` sat after `if (place === '.')
   * return ...`, so a Digit that switches between the decimal point and a
   * numeral across renders calls a different number of hooks and React throws
   * "Rendered fewer hooks than expected", blanking the screen. `places` is
   * derived from the current value's own digits, so it genuinely does change
   * shape while a number counts; this build only ever passes integers, which is
   * why it was not firing, and that is exactly the sort of latent break that
   * surfaces the first time somebody displays a balance with cents. oxlint
   * flags it as a rules-of-hooks error.
   */
  const isDot = place === '.';
  const valueRoundedToPlace = isDot ? 0 : getValueRoundedToPlace(value, place);
